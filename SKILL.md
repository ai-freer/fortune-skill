...
**脚本边界（先读这条，不要去 scripts/ 里搜参数）**：排盘脚本的时间粒度到流年（`--year` 快查）与报告的流月为止，**不存在流日参数**（没有 `--day` / `liuri` / `targetDate`）。今日运势按以下三步，全程只需一次额外脚本调用：

1. **取今日干支**（确定性）：把"今天"作为出生日期排盘，日柱即今日干支：

   ```bash
   node scripts/bazi-chart.mjs --solar "<今天 YYYY-MM-DD>" --hour 12 --gender male --birthplace "北京"
   ```

   输出 JSON 的 `.pillars.day.ganZhi` 即今日干支、`.pillars.day.wuXing` 为其五行（`--hour/--gender/--birthplace` 取任意合法值即可，此调用只消费日柱）。

2. **取用户命盘**：按上文排盘流程获取原局四柱、日主强弱与喜忌（已有命盘数据时直接复用，不重排）。

3. **流日分析（LLM 负责解释）**：今日干支对照原局——
   - 五行生克：今日干支五行 vs 日主与喜用神（喜用同气为吉、忌神当值需谨慎）；
   - 刑冲合害：今日地支与原局四支的六合/三合/六冲/相刑/相害（查 `references/wuxing-tables.md`）；
   - 背景叠加：置于当前大运、流年语境下综合定性。

**粒度约束**：流日只做当天定性输出（宜/忌/注意事项 2–4 条，简短口语化 + 免责声明），不做超出流月颗粒度的具体事件断言。
